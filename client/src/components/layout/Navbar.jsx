...
// ⭐ Generate dynamic categories (excluding filters)
const CATEGORY_LINKS = Object.keys(productsMaster)
  .filter((key) => key !== "filters") // remove filters object
  .map((key) => ({
    name: key.charAt(0).toUpperCase() + key.slice(1).replace(/-/g, " "), // "electronics" -> "Electronics"
    path: `/category/${key}`,
  }));
  ...
  return (
    <header className="sticky top-0 z-50 bg-white font-sans shadow-lg transition-all duration-300">
      ...
      <div className="bg-white py-3 px-4 md:px-8 flex items-center gap-4 md:gap-8 relative">
        ...
        {/* Location - Desktop */}
        <div className="hidden lg:flex flex-col leading-tight cursor-pointer hover:bg-gray-50 p-2 rounded-lg transition-all duration-200 group">
        ...
        <div className="flex-1 max-w-2xl relative z-20">
          <div className="relative group">
            <input
              ...
              className="w-full border-2 border-gray-100 bg-gray-50 py-2.5 pl-5 pr-12 rounded-xl focus:outline-none focus:border-indigo-500 focus:bg-white focus:shadow-lg text-gray-700 font-medium placeholder-gray-400"
            />
            <button className="absolute right-2 top-1.5 bg-indigo-50 text-indigo-600 p-1.5 rounded-lg hover:bg-indigo-600 hover:text-white">
        ...
        <div className="flex items-center gap-2 md:gap-6 text-gray-600 ml-auto md:ml-0">
          ...
          >
            ...
            <span className="absolute -top-0.5 right-0.5 bg-yellow-400 text-indigo-900 text-[10px] w-5 h-5 flex items-center justify-center rounded-full border-2 border-white">
              2
            </span>
          </Link>
        </div>
      ...
      <div className="bg-indigo-900 text-white shadow-inner relative z-10">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-1 overflow-x-auto hide-scrollbar text-sm font-medium">
            ...
            <Link
              to="/products"
              className="flex items-center gap-2 py-3 px-3 mr-4 hover:bg-white/10 border-r border-indigo-800 pr-6"
            >
            ...
            <div className="flex items-center gap-1">
              {CATEGORY_LINKS.map((link, index) => (
                <Link
                  key={index}
                  to={link.path}
                  className="py-3 px-4 whitespace-nowrap text-indigo-100 hover:bg-white/10 hover:text-white rounded-md"
                >
                  {link.name}
                </Link>
              ))}
            ...
            <Link
              to="/todays-special"
              className="ml-auto py-3 px-4 text-yellow-300 font-bold hover:bg-white/10 flex items-center gap-2"
            >
